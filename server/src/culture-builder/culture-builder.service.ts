...
export class CultureBuilderService {
  ...
  async generateWeeklyAppreciationThread(): Promise<Post> {
    ...
    // Generate AI content
    const content =
      await this.aiService.generateAppreciationContent(contributors);

    // Get system user and thread
    const systemUser = await this.getSystemUser();
    const thread = await this.getOrCreateThread(
      this.CULTURE_THREAD_NAME,
  ...
  async generateWeeklyChallenge(): Promise<Post> {
    ...
    const challengeData: ChallengeData = this.createChallengeData(
      challengeType,
      analytics,
    );
    ...
    // Generate AI content
    const content =
      await this.aiService.generateChallengeContent(challengeData);

    // Get system user and thread
    const systemUser = await this.getSystemUser();
    const thread = await this.getOrCreateThread(
      this.CHALLENGE_THREAD_NAME,
  ...
  async deactivatePreviousChallenges(): Promise<void> {
    ...
    if (activeChallenges.length > 0) {
      ...
      this.logger.log(
        `Deactivated ${activeChallenges.length} previous challenge(s)`,
      );
    }
  }
  ...
  private determineChallengeType(analytics: any): ChallengeData['type'] {
    ...
    const types: ChallengeData['type'][] = [
      'engagement',
      'cross-bu',
      'mentorship',
      'knowledge-share',
  ...
  private createChallengeData(
    ...
    const challenges = {
      engagement: {
        ...
        description:
          'This week, challenge yourself to reply to at least 3 posts outside your usual topics. Help someone learn something new!',
      },
      'cross-bu': {
      ...
      mentorship: {
        ...
        description:
          'Share a lesson you learned this week or mentor someone by answering their questions. Knowledge grows when shared!',
      },
      'knowledge-share': {
        title: '📚 Knowledge Sharing Sprint',
        description:
          'Document something you know well! Write a post about a tool, technique, or insight that could help others.',
      },
    };
