...
import { Quiz, QuizSubmission, QuizResult } from './types/culture-builder.types';
...
export class CultureQuizService {
    ...
    async submitQuiz(userId: number, submission: QuizSubmission): Promise<QuizResult> {
        const quiz = await this.quizRepo.findOne({ where: { id: submission.quizId } });
